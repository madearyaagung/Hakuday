...
paraSample.settings = {
    ...
    removeScrollbar: utilLib.deviceDescription.type != utilLib.DEVICE_TYPES.wPhone,
...
function parallax(param) {
    ...
    function layer($src, prnt) {
        ...
        var slowness = 1 - this.spd,
            extraSpeed = this.spd - 1;

        var halfWindowWidth, halfParentWidth, preCalculatedPosition, halfWidth;

        var hasOverflowMarker = $src.hasClass(overflowsParentClass),
            isSmallAndSlow;
    ...
    function slide($src, masterSlide, prnt) {
        ...
        this.adjust = function() {
            ...
            var toTheLeftOfScreen = this.left < 0 && right < 0,
                toTheRightOfScreen = this.left > windowWidth_cache && right > windowWidth_cache;
    ...
    function initScrollbar() {
        ...
        if (param.touchNotScrollMode) {
            ...
            var maxScroll = slides.width - windowWidth,
                minScroll = 0;
    ...
    function closerGeneric(left) {
        var cur = scroll.get(),
            roun = left ? Math.floor : Math.ceil,
            curIndex = cur / slides.singleSlideWidth,
            dest = roun(cur / slides.singleSlideWidth);
...
var hashProcessingSystem = {
    ...
    applyHashFromAddressLine: function() {
        ...
        var addr = self.location.toString(),
            selectedSlide = addr.slice(addr.indexOf('#') + 1);
